...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Caishen: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Caishen Signed Message:\n";
...
{
    // Caishen: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 0)
        ...
        {
           nSubsidy = 560000 * COIN;
	    }
	else if (nHeight > 358363)
      {
         nSubsidy = 10 * COIN;
       }
	else if (nHeight > 1075089)
      {
         nSubsidy = 5 * COIN;
       }
	else if (nHeight > 2150178)
      {
         nSubsidy = 3 * COIN;
       }
	else if (nHeight > 3583630)
      {
         nSubsidy = 1 * COIN;
       }
...
static const int64 nTargetTimespan = 5 * 60; // Caishen: 3.5 days
static const int64 nTargetSpacing = 88; // Caishen: 88 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Caishen: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64        BlocksTargetSpacing                        = 88; //88 second blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 10) { DiffMode = 2; } // caishen, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 500) { DiffMode = 2; }  //caishen, start using kgw after 10ish hours
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Caishen: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Caishen: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Caishen: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaishenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaishenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CaishenMiner(CWallet *pwallet)
{
    printf("CaishenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("caishen-miner");
    ...
    try { loop {
        ...
        printf("Running CaishenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CaishenMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CaishenMiner, pwallet));
}
